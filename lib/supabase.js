// lib/supabase.js - Updated functions for new tool change fields
import { createClient } from '@supabase/supabase-js'

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY

const createMissingCredentialsClient = () => {
  const message = 'Supabase credentials are missing. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY.'
  console.warn(message)

  return new Proxy(
    {},
    {
      get() {
        throw new Error(message)
      }
    }
  )
}

export const supabase = supabaseUrl && supabaseAnonKey
  ? createClient(supabaseUrl, supabaseAnonKey)
  : createMissingCredentialsClient()

const DEFAULT_DOWNTIME_MINUTES = 10

const parseOptionalNumber = (value) => {
  if (value === null || value === undefined) {
    return null
  }

  if (typeof value === 'string' && value.trim() === '') {
    return null
  }

  const parsed = Number(value)
  return Number.isFinite(parsed) ? parsed : null
}

const getDowntimeMinutesWithDefault = (...values) => {
  for (const value of values) {
    const parsed = parseOptionalNumber(value)
    if (parsed !== null) {
      return parsed
    }
  }

  return DEFAULT_DOWNTIME_MINUTES
}

// Get all active operators
export async function getOperators() {
  try {
    console.log('🔍 Fetching operators from database...')

    const { data, error } = await supabase
      .from('operators')
      .select(`
        id,
        employee_id,
        clock_number,
        first_name,
        last_name,
        full_name,
        department,
        primary_shift,
        job_title,
        skill_level,
        cnc_certified,
        lathe_certified,
        mill_certified,
        cat536_6763_experience,
        cat536_6763_pieces_produced
      `)
      .eq('active', true)
      .order('full_name')

    if (error) {
      console.error('❌ Supabase error fetching operators:', error)
      throw error
    }

    console.log('✅ Operators data fetched:', data?.length || 0, 'operators')
    return data || []
  } catch (error) {
    console.error('❌ Error in getOperators:', error)
    return []
  }
}

// Get operators with CAT536-6763 experience (for specific part tracking)
export async function getCAT536Operators() {
  try {
    const { data, error } = await supabase
      .from('operators')
      .select('*')
      .eq('active', true)
      .eq('cat536_6763_experience', true)
      .order('cat536_6763_pieces_produced', { ascending: false })

    if (error) throw error
    return data || []
  } catch (error) {
    console.error('❌ Error fetching CAT536 operators:', error)
    return []
  }
}

// Get operator by employee ID or clock number
export async function getOperatorById(identifier) {
  try {
    let query = supabase.from('operators').select('*').eq('active', true)

    // Check if identifier is numeric (clock number) or alphanumeric (employee ID)
    if (!isNaN(identifier)) {
      query = query.eq('clock_number', parseInt(identifier))
    } else {
      query = query.eq('employee_id', identifier)
    }

    const { data, error } = await query.single()

    if (error) throw error
    return data
  } catch (error) {
    console.error('❌ Error fetching operator by ID:', error)
    return null
  }
}

export async function getAfterBlastMeasurements({ limit = 75 } = {}) {
  try {
    const { data: measurements, error } = await supabase
      .from('dimensional_measurements')
      .select(
        `
        id,
        created_at,
        heat_number,
        tracking_number,
        product_number,
        product_family,
        measurement_date,
        measurement_time,
        operator,
        shift,
        dimensional_status,
        heat_treat_approved,
        surface_condition,
        material_appearance,
        barrel_diameter_actual,
        barrel_diameter_target,
        barrel_diameter_tolerance,
        flange_diameter_actual,
        flange_diameter_target,
        flange_diameter_tolerance,
        overall_length_actual,
        overall_length_target,
        overall_length_tolerance,
        length_to_flange_actual,
        length_to_flange_target,
        length_to_flange_tolerance,
        notes
      `
      )
      .order('measurement_date', { ascending: false, nullsLast: false })
      .order('measurement_time', { ascending: false, nullsLast: false })
      .order('created_at', { ascending: false })
      .limit(limit)

    if (error) {
      console.error('❌ Error fetching after blast measurements:', error)
      throw error
    }

    const productNumbers = Array.from(
      new Set(
        (measurements || [])
          .map((record) => record?.product_number)
          .filter((value) => typeof value === 'string' && value.trim() !== '')
      )
    )

    let templateMap = {}

    if (productNumbers.length > 0) {
      const { data: templates, error: templateError } = await supabase
        .from('measurement_templates')
        .select('product_number, product_family')
        .in('product_number', productNumbers)

      if (templateError) {
        console.error(
          '❌ Error fetching measurement template metadata:',
          templateError
        )
      } else if (templates?.length) {
        templateMap = templates.reduce((acc, template) => {
          if (template?.product_number) {
            acc[template.product_number] = template
          }
          return acc
        }, {})
      }
    }

    return (measurements || []).map((record) => {
      const template = record?.product_number
        ? templateMap[record.product_number]
        : null

      return {
        ...record,
        product_family:
          record?.product_family ?? template?.product_family ?? null
      }
    })
  } catch (error) {
    console.error('❌ getAfterBlastMeasurements failed:', error)
    return []
  }
}

// REPLACE your addToolChange function with this clean version

export async function addToolChange(toolChangeData) {
  try {
    console.log('🔧 Adding tool change to database:', toolChangeData)

    const toNumberOrNull = (value) => {
      if (value === null || value === undefined || value === '') {
        return null
      }

      const parsed = Number(value)
      return Number.isFinite(parsed) ? parsed : null
    }

    const changeReason =
      toolChangeData.change_reason ||
      [toolChangeData.first_rougher_change_reason, toolChangeData.finish_tool_change_reason]
        .filter(Boolean)
        .join(', ') ||
      null

    const downtimeMinutes = getDowntimeMinutesWithDefault(
      toolChangeData.downtime_minutes,
      toolChangeData.tool_change_duration_minutes
    )

    // Map to your actual database schema field names
    const mappedData = {
      // Basic info - use exact database column names
      date: toolChangeData.date || new Date().toISOString().split('T')[0],
      time: toolChangeData.time || new Date().toTimeString().split(' ')[0].slice(0, 5),
      shift: toNumberOrNull(toolChangeData.shift),

      // Operator info - use current schema
      operator: toolChangeData.operator || null,
      operator_id: toolChangeData.operator_id ?? null,
      supervisor: toolChangeData.supervisor || null,

      // Equipment - align with schema column name
      work_center: toolChangeData.work_center || null,
      equipment_number: toolChangeData.equipment_number || toolChangeData.machine_number || null,
      operation: toolChangeData.operation || null,

      // Job tracking
      part_number: toolChangeData.part_number || null,
      job_number: toolChangeData.job_number || null,
      heat_number: toolChangeData.heat_number || null,
      casting_date: toolChangeData.casting_date || null,
      material_appearance: toolChangeData.material_appearance || null,
      material_risk_score: toNumberOrNull(toolChangeData.material_risk_score),

      // Tool metadata
      tool_type: toolChangeData.tool_type || null,
      tool_position: toolChangeData.tool_position || null,
      insert_type: toolChangeData.insert_type || null,
      insert_grade: toolChangeData.insert_grade || null,

      // Tool data - use current schema field names
      old_tool_id: toolChangeData.old_tool_id || toolChangeData.old_first_rougher || toolChangeData.old_finish_tool || null,
      new_tool_id: toolChangeData.new_tool_id || toolChangeData.new_first_rougher || toolChangeData.new_finish_tool || null,
      old_first_rougher: toolChangeData.old_first_rougher || null,
      new_first_rougher: toolChangeData.new_first_rougher || null,
      first_rougher_action: toolChangeData.first_rougher_action || null,
      old_finish_tool: toolChangeData.old_finish_tool || null,
      new_finish_tool: toolChangeData.new_finish_tool || null,
      finish_tool_action: toolChangeData.finish_tool_action || null,
      old_rougher_material_id: toolChangeData.old_rougher_material_id || null,
      new_rougher_material_id: toolChangeData.new_rougher_material_id || null,
      old_finish_material_id: toolChangeData.old_finish_material_id || null,
      new_finish_material_id: toolChangeData.new_finish_material_id || null,
      old_rougher_supplier: toolChangeData.old_rougher_supplier || null,
      new_rougher_supplier: toolChangeData.new_rougher_supplier || null,
      old_finish_supplier: toolChangeData.old_finish_supplier || null,
      new_finish_supplier: toolChangeData.new_finish_supplier || null,

      // Change reasons
      first_rougher_change_reason: toolChangeData.first_rougher_change_reason || null,
      finish_tool_change_reason: toolChangeData.finish_tool_change_reason || null,
      change_reason: changeReason,
      old_tool_condition: toolChangeData.old_tool_condition || null,

      // Performance data
      pieces_produced: toNumberOrNull(toolChangeData.pieces_produced),
      cycle_time_before: toNumberOrNull(toolChangeData.cycle_time_before),
      cycle_time_after: toNumberOrNull(toolChangeData.cycle_time_after),
      cycle_time: toNumberOrNull(toolChangeData.cycle_time),
      downtime_minutes: downtimeMinutes,

      // Quality checks
      dimension_check: toolChangeData.dimension_check || null,
      surface_finish: toolChangeData.surface_finish || null,

      // Cost tracking
      rougher_cost: toNumberOrNull(toolChangeData.rougher_cost),
      finish_cost: toNumberOrNull(toolChangeData.finish_cost),
      total_tool_cost: toNumberOrNull(toolChangeData.total_tool_cost),

      // Metadata
      notes: toolChangeData.notes || null,
      created_at: new Date().toISOString(),
    }

    const sanitizedData = Object.fromEntries(
      Object.entries(mappedData).filter(([_, value]) => value !== undefined)
    )

    console.log('📝 Mapped data for database:', sanitizedData)

    const insertResult = await saveToolChangeRecord(sanitizedData)

    console.log('✅ Tool change added successfully:', insertResult)
    return insertResult
  } catch (error) {
    console.error('❌ Error in addToolChange:', error)
    throw error
  }
}

const unsupportedToolChangeColumns = new Set()

const shouldUseToolChangeApi = () => typeof window !== 'undefined' && typeof fetch === 'function'

const filterUnsupportedColumns = (data) =>
  Object.fromEntries(
    Object.entries(data).filter(([column]) => !unsupportedToolChangeColumns.has(column))
  )

const insertToolChangeViaApi = async (payload) => {
  const response = await fetch('/api/tool-changes', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(payload)
  })

  const contentType = response.headers.get('content-type') || ''
  const isJson = contentType.includes('application/json')
  const body = isJson ? await response.json() : null

  if (!response.ok) {
    const error = new Error(body?.error || `Tool change API request failed with status ${response.status}`)
    error.status = response.status
    if (body?.code) error.code = body.code
    if (body?.details) error.details = body.details
    if (body?.hint) error.hint = body.hint
    throw error
  }

  return body?.data || null
}

async function saveToolChangeRecord(payload) {
  if (shouldUseToolChangeApi()) {
    try {
      return await insertToolChangeViaApi(payload)
    } catch (error) {
      if (error?.status && error.status !== 404 && error.status !== 405) {
        throw error
      }

      console.warn('⚠️ Tool change API unavailable. Falling back to direct Supabase insert.', error)
    }
  }

  return insertToolChangeWithFallback(payload)
}

const normalizeSupabaseError = (error, { status } = {}) => {
  if (error instanceof Error) {
    if (status !== undefined) {
      error.status = status
    }
    return error
  }

  const normalizedError = new Error(error?.message || 'Supabase request failed')

  if (status !== undefined) {
    normalizedError.status = status
  }

  if (error && typeof error === 'object') {
    if (error.code) normalizedError.code = error.code
    if (error.details) normalizedError.details = error.details
    if (error.hint) normalizedError.hint = error.hint
  }

  return normalizedError
}

const TOOL_REPLACEMENT_ACTIONS = new Set(['REPLACE', 'New Insert'])

export async function submitToolChangeWithCosts(formData) {
  try {
    const enhancedData = await enrichToolChangeData(formData)

    if (enhancedData.rougher_cost != null || enhancedData.finish_cost != null) {
      const rougherCost = Number.parseFloat(enhancedData.rougher_cost) || 0
      const finishCost = Number.parseFloat(enhancedData.finish_cost) || 0
      const totalCost = rougherCost + finishCost

      if (Number.isFinite(totalCost) && totalCost > 0) {
        enhancedData.total_tool_cost = totalCost
      }
    }

    const result = await addToolChange(enhancedData)

    await updateToolInventory(enhancedData)

    return result
  } catch (error) {
    console.error('❌ Error saving tool change:', error)
    throw error
  }
}

async function enrichToolChangeData(formData) {
  const enrichedData = { ...formData }

  if (
    formData?.new_first_rougher &&
    formData?.first_rougher_action &&
    TOOL_REPLACEMENT_ACTIONS.has(formData.first_rougher_action)
  ) {
    const rougherCost = await getToolCost(
      formData?.insert_type,
      formData?.insert_grade,
      formData?.new_rougher_supplier
    )

    if (rougherCost) {
      enrichedData.rougher_cost = rougherCost.cost_per_tool
      enrichedData.new_rougher_material_id =
        enrichedData.new_rougher_material_id || rougherCost.material_id
    }
  }

  if (
    formData?.new_finish_tool &&
    formData?.finish_tool_action &&
    TOOL_REPLACEMENT_ACTIONS.has(formData.finish_tool_action)
  ) {
    const finishCost = await getToolCost(
      formData?.insert_type,
      formData?.insert_grade,
      formData?.new_finish_supplier
    )

    if (finishCost) {
      enrichedData.finish_cost = finishCost.cost_per_tool
      enrichedData.new_finish_material_id =
        enrichedData.new_finish_material_id || finishCost.material_id
    }
  }

  return enrichedData
}

async function getToolCost(insertType, insertGrade, supplier) {
  if (!insertType || !insertGrade || !supplier) {
    return null
  }

  const { data, error } = await supabase
    .from('tool_costs')
    .select('cost_per_tool, material_id')
    .eq('insert_type', insertType)
    .eq('insert_grade', insertGrade)
    .eq('supplier', supplier)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle()

  if (error) {
    console.warn('Could not find tool cost:', error)
    return null
  }

  return data
}

async function updateToolInventory(toolChangeData) {
  const adjustments = []

  if (toolChangeData?.new_rougher_material_id) {
    adjustments.push(adjustInventory(toolChangeData.new_rougher_material_id, -1))
  }

  if (toolChangeData?.new_finish_material_id) {
    adjustments.push(adjustInventory(toolChangeData.new_finish_material_id, -1))
  }

  if (adjustments.length === 0) {
    return
  }

  await Promise.allSettled(adjustments)
}

async function adjustInventory(materialId, quantityChange) {
  if (!materialId || !Number.isFinite(Number(quantityChange))) {
    return
  }

  const { error } = await supabase.rpc('adjust_tool_inventory', {
    p_material_id: materialId,
    p_quantity_change: quantityChange,
  })

  if (error) {
    console.warn('Inventory update failed:', error)
  }
}

async function insertToolChangeWithFallback(initialData) {
  let payload = filterUnsupportedColumns(initialData)
  const removedColumns = new Set()

  while (true) {
    const { data, error, status } = await supabase
      .from('tool_changes')
      .insert([payload])
      .select()

    if (!error) {
      if (removedColumns.size > 0) {
        console.warn(
          `⚠️ Insert succeeded after removing unsupported columns: ${Array.from(removedColumns).join(', ')}`
        )
      }
      return data
    }

    if (status === 401 || status === 403) {
      throw normalizeSupabaseError(error, { status })
    }

    const missingColumnMatch =
      error?.code === 'PGRST204' && typeof error?.message === 'string'
        ? error.message.match(/'([^']+)' column/)
        : null

    if (missingColumnMatch) {
      const missingColumn = missingColumnMatch[1]

      if (missingColumn && Object.prototype.hasOwnProperty.call(payload, missingColumn)) {
        console.warn(`⚠️ Column '${missingColumn}' not found in schema. Retrying without it...`)
        unsupportedToolChangeColumns.add(missingColumn)
        delete payload[missingColumn]
        removedColumns.add(missingColumn)
        continue
      }
    }

    console.error('❌ Supabase error:', error)
    throw normalizeSupabaseError(error, { status })
  }
}

// Get all tool changes with updated fields
export async function getToolChanges() {
  try {
    const { data, error } = await supabase
      .from('tool_changes')
      .select('*')
      .order('created_at', { ascending: false });

    if (error) {
      console.error('❌ Error fetching tool changes:', error);
      throw error;
    }

    return data;
  } catch (error) {
    console.error('❌ Error in getToolChanges:', error);
    throw error;
  }
}

// Get enhanced tool changes with operator details
export async function getToolChangesWithOperators() {
  try {
    const { data, error } = await supabase
      .from('tool_changes')
      .select(`
        *,
        operator_details:operator_id (
          employee_id,
          clock_number,
          full_name,
          skill_level,
          cat536_6763_experience
        )
      `)
      .order('created_at', { ascending: false });

    if (error) {
      console.error('❌ Error fetching tool changes with operators:', error);
      throw error;
    }

    return data;
  } catch (error) {
    console.error('❌ Error in getToolChangesWithOperators:', error);
    throw error;
  }
}

// Get tool changes by date range
export async function getToolChangesByDateRange(startDate, endDate) {
  try {
    const { data, error } = await supabase
      .from('tool_changes')
      .select('*')
      .gte('date', startDate)
      .lte('date', endDate)
      .order('date', { ascending: false });

    if (error) throw error;
    return data;
  } catch (error) {
    console.error('❌ Error fetching tool changes by date range:', error);
    throw error;
  }
}

// Get tool changes by operator
export async function getToolChangesByOperator(operator) {
  try {
    const { data, error } = await supabase
      .from('tool_changes')
      .select('*')
      .eq('operator', operator)
      .order('created_at', { ascending: false });

    if (error) throw error;
    return data;
  } catch (error) {
    console.error('❌ Error fetching tool changes by operator:', error);
    throw error;
  }
}

// Get tool changes by insert type (new function for insert analysis)
export async function getToolChangesByInsertType(insertType, insertCategory = null) {
  try {
    let query = supabase
      .from('tool_changes')
      .select('*')
      .order('created_at', { ascending: false });

    if (insertCategory === 'rougher') {
      query = query.or(
        `new_first_rougher.eq.${insertType},old_first_rougher.eq.${insertType},first_rougher.eq.${insertType}`
      );
    } else if (insertCategory === 'finish') {
      query = query.or(
        `new_finish_tool.eq.${insertType},old_finish_tool.eq.${insertType},finish_tool.eq.${insertType}`
      );
    } else {
      // Search both rougher and finish columns
      query = query.or(
        [
          `new_first_rougher.eq.${insertType}`,
          `old_first_rougher.eq.${insertType}`,
          `first_rougher.eq.${insertType}`,
          `new_finish_tool.eq.${insertType}`,
          `old_finish_tool.eq.${insertType}`,
          `finish_tool.eq.${insertType}`
        ].join(',')
      );
    }

    const { data, error } = await query;

    if (error) throw error;
    return data;
  } catch (error) {
    console.error('❌ Error fetching tool changes by insert type:', error);
    throw error;
  }
}

// Get equipment list
export async function getEquipment() {
  try {
    console.log('🔍 Fetching equipment from database...');
    
    const { data, error } = await supabase
      .from('equipment')
      .select('*')
      .eq('active', true)
      .order('equipment_number');

    if (error) {
      console.error('❌ Supabase error fetching equipment:', error);
      throw error;
    }

    console.log('✅ Equipment data fetched:', data);
    return data || [];
  } catch (error) {
    console.error('❌ Error in getEquipment:', error);
    return [];
  }
}

// Get tool inventory
export async function getToolInventory() {
  try {
    console.log('🔍 Fetching tool inventory from database...');
    
    const { data, error } = await supabase
      .from('tool_inventory')
      .select('*')
      .eq('active', true)
      .order('tool_id');

    if (error) {
      console.error('❌ Supabase error fetching tools:', error);
      throw error;
    }

    console.log('✅ Tool inventory data fetched:', data);
    return data || [];
  } catch (error) {
    console.error('❌ Error in getToolInventory:', error);
    return [];
  }
}

// Insert usage analytics - updated for new fields
export async function getInsertUsageAnalytics(dateRange = 30) {
  try {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - dateRange);
    
    const { data, error } = await supabase
      .from('tool_changes')
      .select(`
        new_first_rougher,
        old_first_rougher,
        first_rougher,
        new_finish_tool,
        old_finish_tool,
        finish_tool,
        change_reason,
        pieces_produced,
        cycle_time_before,
        cycle_time_after,
        downtime_minutes,
        tool_change_duration_minutes,
        date,
        operator,
        equipment_number
      `)
      .gte('date', startDate.toISOString().split('T')[0])
      .order('date', { ascending: false });

    if (error) throw error;

    // Process data to create analytics
    const analytics = {
      rougherUsage: {},
      finishUsage: {},
      totalChanges: data.length,
      averageDowntime: 0,
      topChangeReasons: {},
      operatorPerformance: {},
      equipmentUsage: {}
    };

    const toNumber = value => {
      const numericValue = Number(value);
      return Number.isFinite(numericValue) ? numericValue : 0;
    };

    let totalDowntime = 0;
    let downtimeCount = 0;

    data.forEach(change => {
      // Track rougher insert usage
      const rougherInsert =
        change.new_first_rougher ||
        change.old_first_rougher ||
        change.first_rougher ||
        null;

      if (rougherInsert) {
        analytics.rougherUsage[rougherInsert] =
          (analytics.rougherUsage[rougherInsert] || 0) + 1;
      }

      // Track finish insert usage
      const finishInsert =
        change.new_finish_tool ||
        change.old_finish_tool ||
        change.finish_tool ||
        null;

      if (finishInsert) {
        analytics.finishUsage[finishInsert] =
          (analytics.finishUsage[finishInsert] || 0) + 1;
      }

      // Track change reasons
      if (change.change_reason) {
        analytics.topChangeReasons[change.change_reason] = 
          (analytics.topChangeReasons[change.change_reason] || 0) + 1;
      }

      // Track operator performance
      if (change.operator) {
        if (!analytics.operatorPerformance[change.operator]) {
          analytics.operatorPerformance[change.operator] = {
            changes: 0,
            totalPieces: 0,
            totalDowntime: 0
          };
        }
        analytics.operatorPerformance[change.operator].changes++;
        analytics.operatorPerformance[change.operator].totalPieces +=
          toNumber(change.pieces_produced);
        const operatorDowntime = getDowntimeMinutesWithDefault(
          change.downtime_minutes,
          change.tool_change_duration_minutes
        )

        analytics.operatorPerformance[change.operator].totalDowntime +=
          operatorDowntime
      }

      // Track equipment usage
      if (change.equipment_number) {
        analytics.equipmentUsage[change.equipment_number] =
          (analytics.equipmentUsage[change.equipment_number] || 0) + 1;
      }

      // Calculate average downtime
      const downtimeValue = getDowntimeMinutesWithDefault(
        change.downtime_minutes,
        change.tool_change_duration_minutes
      )

      if (Number.isFinite(downtimeValue)) {
        totalDowntime += downtimeValue;
        downtimeCount++;
      }
    });

    analytics.averageDowntime = downtimeCount > 0 ?
      Number((totalDowntime / downtimeCount).toFixed(2)) : 0;

    return analytics;
  } catch (error) {
    console.error('❌ Error in getInsertUsageAnalytics:', error);
    throw error;
  }
}

// Cost analysis function - updated for new insert types
export async function getToolCostAnalysis(dateRange = 30, options = {}) {
  try {
    const sanitizedRange = Number.isFinite(dateRange) && dateRange > 0 ? dateRange : 30;

    const today = new Date();
    today.setHours(23, 59, 59, 999);
    const startDate = new Date(today);
    startDate.setHours(0, 0, 0, 0);
    startDate.setDate(startDate.getDate() - sanitizedRange);

    const { preloadedChanges = null } = options ?? {};

    const isWithinRange = changeDate => {
      if (!(changeDate instanceof Date) || Number.isNaN(changeDate.getTime())) {
        return true;
      }
      return changeDate >= startDate && changeDate <= today;
    };

    const filterChangesByDate = (changes = []) =>
      changes.filter(change => {
        if (!change?.date) return true;
        const changeDate = new Date(change.date);
        return isWithinRange(changeDate);
      });

    let changes = Array.isArray(preloadedChanges)
      ? filterChangesByDate(preloadedChanges)
      : null;

    if (!changes) {
      const { data: changesData, error: changesError } = await supabase
        .from('tool_changes')
        .select(`
          new_first_rougher,
          old_first_rougher,
          first_rougher,
          new_finish_tool,
          old_finish_tool,
          finish_tool,
          pieces_produced,
          downtime_minutes,
          tool_change_duration_minutes,
          rougher_cost,
          finish_cost,
          total_tool_cost,
          date,
          equipment_number
        `)
        .gte('date', startDate.toISOString().split('T')[0]);

      if (changesError) {
        console.warn(
          '⚠️ Error applying date filter in getToolCostAnalysis. Falling back to local filtering.',
          changesError
        );

        const { data: fallbackChanges, error: fallbackError } = await supabase
          .from('tool_changes')
          .select(`
            new_first_rougher,
            old_first_rougher,
            first_rougher,
            new_finish_tool,
            old_finish_tool,
            finish_tool,
            pieces_produced,
            downtime_minutes,
            tool_change_duration_minutes,
            rougher_cost,
            finish_cost,
            total_tool_cost,
            date,
            equipment_number
          `);

        if (fallbackError) throw fallbackError;

        changes = filterChangesByDate(fallbackChanges || []);
      } else {
        changes = changesData || [];
      }
    }

    // Get tool costs
    let costs = [];
    const { data: costsData, error: costsError } = await supabase
      .from('tool_costs')
      .select('*');

    if (costsError) {
      const missingRelation =
        costsError?.code === '42P01' ||
        costsError?.message?.toLowerCase().includes('does not exist');

      if (missingRelation) {
        console.warn(
          '⚠️ tool_costs table not found. Falling back to default cost values.'
        );
      } else {
        throw costsError;
      }
    } else {
      costs = costsData || [];
    }

    // Calculate cost analysis
    let totalInsertCost = 0;
    let totalDowntimeCost = 0;
    const downtimeHourlyRate = 150; // $150/hour machine rate

    const parseCostValue = value => {
      if (value === null || value === undefined) return null;
      const numericValue = typeof value === 'number' ? value : parseFloat(value);
      return Number.isFinite(numericValue) ? numericValue : null;
    };

    changes.forEach(change => {
      // Calculate insert costs prioritizing stored values
      const recordedRougherCost = parseCostValue(change.rougher_cost);
      const recordedFinishCost = parseCostValue(change.finish_cost);
      const recordedTotalCost = parseCostValue(change.total_tool_cost);

      let changeInsertCost = 0;

      if (recordedRougherCost !== null || recordedFinishCost !== null) {
        changeInsertCost += (recordedRougherCost || 0) + (recordedFinishCost || 0);
      } else if (recordedTotalCost !== null) {
        changeInsertCost += recordedTotalCost;
      } else {
        const rougherInsert =
          change.new_first_rougher ||
          change.old_first_rougher ||
          change.first_rougher ||
          null;
        const rougherCostEntry = rougherInsert
          ? costs.find(c => c.insert_type === rougherInsert)
          : null;
        const rougherCost = rougherCostEntry?.cost_per_tool || 25; // Default $25

        const finishInsert =
          change.new_finish_tool ||
          change.old_finish_tool ||
          change.finish_tool ||
          null;
        const finishCostEntry = finishInsert
          ? costs.find(c => c.insert_type === finishInsert)
          : null;
        const finishCost = finishCostEntry?.cost_per_tool || 30; // Default $30

        changeInsertCost += rougherCost + finishCost;
      }

      totalInsertCost += changeInsertCost;

      // Calculate downtime costs
      const downtimeMinutesForChange = getDowntimeMinutesWithDefault(
        change.downtime_minutes,
        change.tool_change_duration_minutes
      )

      if (Number.isFinite(downtimeMinutesForChange)) {
        totalDowntimeCost += (downtimeMinutesForChange / 60) * downtimeHourlyRate
      }
    });

    return {
      totalInsertCost: totalInsertCost.toFixed(2),
      totalDowntimeCost: totalDowntimeCost.toFixed(2),
      totalCost: (totalInsertCost + totalDowntimeCost).toFixed(2),
      averageCostPerChange: changes.length > 0 ? 
        ((totalInsertCost + totalDowntimeCost) / changes.length).toFixed(2) : 0,
      periodDays: dateRange,
      totalChanges: changes.length
    };
  } catch (error) {
    console.error('❌ Error in getToolCostAnalysis:', error);
    throw error;
  }
}

// Get operator performance analytics
export async function getOperatorPerformanceAnalytics(dateRange = 30) {
  try {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - dateRange);

    const { data, error } = await supabase
      .from('tool_changes')
      .select(`
        operator,
        operator_id,
        pieces_produced,
        downtime_minutes,
        tool_change_duration_minutes,
        cycle_time_before,
        cycle_time_after,
        change_reason,
        date,
        operator_details:operator_id (
          employee_id,
          full_name,
          skill_level,
          cat536_6763_pieces_produced
        )
      `)
      .gte('date', startDate.toISOString().split('T')[0])
      .order('date', { ascending: false });

    if (error) throw error;

    // Process the data to create performance metrics
    const operatorStats = {};

    data.forEach(change => {
      const operatorKey = change.operator_details?.employee_id || change.operator || 'Unknown';

      if (!operatorStats[operatorKey]) {
        operatorStats[operatorKey] = {
          name: change.operator_details?.full_name || change.operator,
          skill_level: change.operator_details?.skill_level || 'Unknown',
          cat536_experience: change.operator_details?.cat536_6763_pieces_produced || 0,
          tool_changes: 0,
          total_pieces: 0,
          total_downtime: 0,
          avg_cycle_improvement: 0,
          cycle_improvements: []
        };
      }

      const stats = operatorStats[operatorKey];
      stats.tool_changes++;
      stats.total_pieces += change.pieces_produced || 0;

      const downtimeMinutes = getDowntimeMinutesWithDefault(
        change.downtime_minutes,
        change.tool_change_duration_minutes
      )

      if (Number.isFinite(downtimeMinutes)) {
        stats.total_downtime += downtimeMinutes;
      }

      // Calculate cycle time improvement
      if (change.cycle_time_before && change.cycle_time_after) {
        const improvement = change.cycle_time_before - change.cycle_time_after;
        stats.cycle_improvements.push(improvement);
      }
    });

    // Calculate averages
    Object.values(operatorStats).forEach(stats => {
      stats.avg_pieces_per_change = stats.tool_changes > 0 ?
        Math.round(stats.total_pieces / stats.tool_changes) : 0;
      stats.avg_downtime_per_change = stats.tool_changes > 0 ?
        Math.round(stats.total_downtime / stats.tool_changes) : 0;
      stats.avg_cycle_improvement = stats.cycle_improvements.length > 0 ?
        stats.cycle_improvements.reduce((a, b) => a + b, 0) / stats.cycle_improvements.length : 0;
    });

    return operatorStats;
  } catch (error) {
    console.error('❌ Error in getOperatorPerformanceAnalytics:', error);
    throw error;
  }
}

// ===============
// MACHINE RUNTIME TRACKING FUNCTIONS
// ===============

/**
 * Get machine runtime between tool changes
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {string} startDate - Start date (YYYY-MM-DD)
 * @param {string} endDate - End date (YYYY-MM-DD)
 * @returns {Promise<Array>} Runtime data between tool changes
 */
export async function getMachineRuntimeBetweenToolChanges(equipmentNumber = null, startDate = null, endDate = null) {
  try {
    console.log('🔍 Fetching machine runtime between tool changes...')

    const { data, error } = await supabase.rpc('get_machine_runtime_between_tool_changes', {
      equipment_num: equipmentNumber,
      start_date: startDate,
      end_date: endDate
    })

    if (error) {
      console.error('❌ Supabase error fetching runtime data:', error)
      throw error
    }

    console.log('✅ Runtime data fetched:', data?.length || 0, 'records')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimeBetweenToolChanges:', error)
    throw error
  }
}

/**
 * Get machine runtime summary statistics
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {number} daysBack - Number of days to look back (default: 30)
 * @returns {Promise<Array>} Summary statistics for machine runtime
 */
export async function getMachineRuntimeSummary(equipmentNumber = null, daysBack = 30) {
  try {
    console.log('🔍 Fetching machine runtime summary...')

    const { data, error } = await supabase.rpc('get_machine_runtime_summary', {
      equipment_num: equipmentNumber,
      days_back: daysBack
    })

    if (error) {
      console.error('❌ Supabase error fetching runtime summary:', error)
      throw error
    }

    console.log('✅ Runtime summary fetched:', data?.length || 0, 'machines')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimeSummary:', error)
    throw error
  }
}

/**
 * Get machine runtime analytics (daily aggregated view)
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {number} daysBack - Number of days to look back (default: 30)
 * @returns {Promise<Array>} Daily runtime analytics
 */
export async function getMachineRuntimeAnalytics(equipmentNumber = null, daysBack = 30) {
  try {
    console.log('🔍 Fetching machine runtime analytics...')

    const startDate = new Date()
    startDate.setDate(startDate.getDate() - daysBack)
    const formattedStartDate = startDate.toISOString().split('T')[0]

    let query = supabase
      .from('machine_runtime_analytics')
      .select('*')
      .gte('change_date', formattedStartDate)
      .order('change_date', { ascending: false })

    if (equipmentNumber) {
      query = query.eq('equipment_number', equipmentNumber)
    }

    const { data, error } = await query

    if (error) {
      console.error('❌ Supabase error fetching runtime analytics:', error)
      throw error
    }

    console.log('✅ Runtime analytics fetched:', data?.length || 0, 'records')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimeAnalytics:', error)
    throw error
  }
}

/**
 * Get machine runtime by shift
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {number} daysBack - Number of days to look back (default: 30)
 * @returns {Promise<Array>} Runtime data grouped by shift
 */
export async function getMachineRuntimeByShift(equipmentNumber = null, daysBack = 30) {
  try {
    console.log('🔍 Fetching machine runtime by shift...')

    const startDate = new Date()
    startDate.setDate(startDate.getDate() - daysBack)
    const formattedStartDate = startDate.toISOString().split('T')[0]

    let query = supabase
      .from('machine_runtime_by_shift')
      .select('*')
      .gte('change_date', formattedStartDate)
      .order('change_date', { ascending: false })

    if (equipmentNumber) {
      query = query.eq('equipment_number', equipmentNumber)
    }

    const { data, error } = await query

    if (error) {
      console.error('❌ Supabase error fetching runtime by shift:', error)
      throw error
    }

    console.log('✅ Runtime by shift fetched:', data?.length || 0, 'records')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimeByShift:', error)
    throw error
  }
}

/**
 * Get machine runtime by operator
 * @param {string} operator - Optional operator name to filter
 * @param {number} monthsBack - Number of months to look back (default: 3)
 * @returns {Promise<Array>} Runtime data grouped by operator
 */
export async function getMachineRuntimeByOperator(operator = null, monthsBack = 3) {
  try {
    console.log('🔍 Fetching machine runtime by operator...')

    const startDate = new Date()
    startDate.setMonth(startDate.getMonth() - monthsBack)
    const formattedStartDate = startDate.toISOString().split('T')[0]

    let query = supabase
      .from('machine_runtime_by_operator')
      .select('*')
      .gte('month_start', formattedStartDate)
      .order('month_start', { ascending: false })

    if (operator) {
      query = query.eq('operator', operator)
    }

    const { data, error } = await query

    if (error) {
      console.error('❌ Supabase error fetching runtime by operator:', error)
      throw error
    }

    console.log('✅ Runtime by operator fetched:', data?.length || 0, 'records')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimeByOperator:', error)
    throw error
  }
}

/**
 * Calculate runtime metrics for a specific time period
 * @param {Array} runtimeData - Runtime data from getMachineRuntimeBetweenToolChanges
 * @returns {Object} Calculated metrics
 */
export function calculateRuntimeMetrics(runtimeData) {
  if (!runtimeData || runtimeData.length === 0) {
    return {
      totalRuntime: 0,
      averageRuntime: 0,
      minRuntime: 0,
      maxRuntime: 0,
      totalPieces: 0,
      piecesPerHour: 0,
      totalDowntime: 0,
      utilizationRate: 0
    }
  }

  const totalRuntime = runtimeData.reduce((sum, record) => sum + (parseFloat(record.runtime_hours) || 0), 0)
  const totalPieces = runtimeData.reduce((sum, record) => sum + (parseInt(record.pieces_produced) || 0), 0)
  const totalDowntime = runtimeData.reduce((sum, record) => sum + (parseInt(record.downtime_minutes) || 0), 0)

  const runtimeHours = runtimeData
    .map(r => parseFloat(r.runtime_hours))
    .filter(h => h && h > 0)

  const minRuntime = runtimeHours.length > 0 ? Math.min(...runtimeHours) : 0
  const maxRuntime = runtimeHours.length > 0 ? Math.max(...runtimeHours) : 0
  const averageRuntime = totalRuntime / runtimeData.length

  const piecesPerHour = totalRuntime > 0 ? totalPieces / totalRuntime : 0

  const totalTimeMinutes = (totalRuntime * 60) + totalDowntime
  const utilizationRate = totalTimeMinutes > 0 ? ((totalRuntime * 60) / totalTimeMinutes) * 100 : 0

  return {
    totalRuntime: totalRuntime.toFixed(2),
    averageRuntime: averageRuntime.toFixed(2),
    minRuntime: minRuntime.toFixed(2),
    maxRuntime: maxRuntime.toFixed(2),
    totalPieces,
    piecesPerHour: piecesPerHour.toFixed(2),
    totalDowntime,
    utilizationRate: utilizationRate.toFixed(2)
  }
}

// ===============
// QR CODE RUNTIME TRACKING FUNCTIONS
// ===============

/**
 * Record a machine state event (START or STOP) from QR code scan
 * @param {Object} eventData - Event details
 * @returns {Promise<Object>} Inserted event record
 */
export async function recordMachineStateEvent(eventData) {
  try {
    console.log('🔍 Recording machine state event:', eventData.event_type)

    const {
      equipment_number,
      event_type,
      event_timestamp = new Date().toISOString(),
      operator,
      operator_id,
      shift,
      work_center,
      part_number,
      job_number,
      qr_code_data,
      notes,
      tool_change_id
    } = eventData

    // Validate event type
    if (!['START', 'STOP'].includes(event_type)) {
      throw new Error('Invalid event_type. Must be START or STOP.')
    }

    // Validate required fields
    if (!equipment_number) {
      throw new Error('equipment_number is required')
    }

    const { data, error } = await supabase
      .from('machine_state_events')
      .insert({
        equipment_number,
        event_type,
        event_timestamp,
        operator,
        operator_id,
        shift,
        work_center,
        part_number,
        job_number,
        qr_code_data,
        notes,
        tool_change_id
      })
      .select()
      .single()

    if (error) {
      console.error('❌ Supabase error recording event:', error)
      throw error
    }

    console.log('✅ Machine state event recorded:', data.id)
    return data
  } catch (error) {
    console.error('❌ Error in recordMachineStateEvent:', error)
    throw error
  }
}

/**
 * Get QR code definition by code
 * @param {string} qrCode - The QR code string
 * @returns {Promise<Object>} QR code definition
 */
export async function getQRCodeDefinition(qrCode) {
  try {
    console.log('🔍 Looking up QR code:', qrCode)

    const { data, error } = await supabase
      .from('qr_code_definitions')
      .select('*')
      .eq('qr_code', qrCode)
      .eq('active', true)
      .single()

    if (error) {
      if (error.code === 'PGRST116') {
        // No rows returned
        return null
      }
      console.error('❌ Supabase error fetching QR code definition:', error)
      throw error
    }

    console.log('✅ QR code definition found:', data.event_type)
    return data
  } catch (error) {
    console.error('❌ Error in getQRCodeDefinition:', error)
    throw error
  }
}

/**
 * Get machine runtime periods (paired START/STOP events)
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {string} startDate - Start date (ISO format)
 * @param {string} endDate - End date (ISO format)
 * @returns {Promise<Array>} Runtime periods
 */
export async function getMachineRuntimePeriods(equipmentNumber = null, startDate = null, endDate = null) {
  try {
    console.log('🔍 Fetching machine runtime periods...')

    const { data, error } = await supabase.rpc('get_machine_runtime_periods', {
      equipment_num: equipmentNumber,
      start_date: startDate,
      end_date: endDate
    })

    if (error) {
      console.error('❌ Supabase error fetching runtime periods:', error)
      throw error
    }

    console.log('✅ Runtime periods fetched:', data?.length || 0, 'periods')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineRuntimePeriods:', error)
    throw error
  }
}

/**
 * Get total runtime between tool changes (sums all START/STOP pairs)
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {string} startDate - Start date (YYYY-MM-DD)
 * @param {string} endDate - End date (YYYY-MM-DD)
 * @returns {Promise<Array>} Total runtime between tool changes
 */
export async function getTotalRuntimeBetweenToolChanges(equipmentNumber = null, startDate = null, endDate = null) {
  try {
    console.log('🔍 Fetching total runtime between tool changes...')

    const { data, error } = await supabase.rpc('get_total_runtime_between_tool_changes', {
      equipment_num: equipmentNumber,
      start_date: startDate,
      end_date: endDate
    })

    if (error) {
      console.error('❌ Supabase error fetching total runtime:', error)
      throw error
    }

    console.log('✅ Total runtime data fetched:', data?.length || 0, 'records')
    return data || []
  } catch (error) {
    console.error('❌ Error in getTotalRuntimeBetweenToolChanges:', error)
    throw error
  }
}

/**
 * Get QR-based runtime summary
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @param {number} daysBack - Number of days to look back (default: 30)
 * @returns {Promise<Array>} Runtime summary statistics
 */
export async function getQRRuntimeSummary(equipmentNumber = null, daysBack = 30) {
  try {
    console.log('🔍 Fetching QR runtime summary...')

    const { data, error } = await supabase.rpc('get_qr_runtime_summary', {
      equipment_num: equipmentNumber,
      days_back: daysBack
    })

    if (error) {
      console.error('❌ Supabase error fetching QR runtime summary:', error)
      throw error
    }

    console.log('✅ QR runtime summary fetched:', data?.length || 0, 'machines')
    return data || []
  } catch (error) {
    console.error('❌ Error in getQRRuntimeSummary:', error)
    throw error
  }
}

/**
 * Get unpaired machine events (START without STOP or vice versa)
 * @param {string} equipmentNumber - Optional equipment number to filter
 * @returns {Promise<Array>} Unpaired events
 */
export async function getUnpairedMachineEvents(equipmentNumber = null) {
  try {
    console.log('🔍 Fetching unpaired machine events...')

    let query = supabase
      .from('unpaired_machine_events')
      .select('*')
      .order('event_timestamp', { ascending: false })

    if (equipmentNumber) {
      query = query.eq('equipment_number', equipmentNumber)
    }

    const { data, error } = await query

    if (error) {
      console.error('❌ Supabase error fetching unpaired events:', error)
      throw error
    }

    console.log('✅ Unpaired events fetched:', data?.length || 0, 'events')
    return data || []
  } catch (error) {
    console.error('❌ Error in getUnpairedMachineEvents:', error)
    throw error
  }
}

/**
 * Manually pair machine state events
 * @returns {Promise<void>}
 */
export async function pairMachineStateEvents() {
  try {
    console.log('🔍 Manually pairing machine state events...')

    const { error } = await supabase.rpc('pair_machine_state_events')

    if (error) {
      console.error('❌ Supabase error pairing events:', error)
      throw error
    }

    console.log('✅ Machine state events paired successfully')
  } catch (error) {
    console.error('❌ Error in pairMachineStateEvents:', error)
    throw error
  }
}

/**
 * Get machine state events with filtering
 * @param {Object} filters - Filter options
 * @returns {Promise<Array>} Machine state events
 */
export async function getMachineStateEvents(filters = {}) {
  try {
    console.log('🔍 Fetching machine state events...')

    const {
      equipmentNumber,
      eventType,
      paired,
      startDate,
      endDate,
      limit = 100
    } = filters

    let query = supabase
      .from('machine_state_events')
      .select('*')
      .order('event_timestamp', { ascending: false })
      .limit(limit)

    if (equipmentNumber) {
      query = query.eq('equipment_number', equipmentNumber)
    }

    if (eventType) {
      query = query.eq('event_type', eventType)
    }

    if (paired !== undefined) {
      query = query.eq('paired', paired)
    }

    if (startDate) {
      query = query.gte('event_timestamp', startDate)
    }

    if (endDate) {
      query = query.lte('event_timestamp', endDate)
    }

    const { data, error } = await query

    if (error) {
      console.error('❌ Supabase error fetching machine state events:', error)
      throw error
    }

    console.log('✅ Machine state events fetched:', data?.length || 0, 'events')
    return data || []
  } catch (error) {
    console.error('❌ Error in getMachineStateEvents:', error)
    throw error
  }
}
