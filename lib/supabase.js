...
export async function addToolChange(toolChangeData) {
  try {
    ...
    // Map data to ONLY the fields that exist in your database
    const mappedData = {
      // Basic information - confirmed to exist
      ...
      // Equipment and operation - confirmed to exist
      ...
      // Tool information - confirmed to exist in your schema
      ...
      // Change reasons - confirmed to exist
      ...
      // Notes - confirmed to exist
      notes: toolChangeData.notes,

      // Timestamp - standard field
      created_at: toolChangeData.created_at || new Date().toISOString(),
    }

    // Remove null/undefined values and fields that don't exist
    ...
    console.log('📦 Clean data for database (only existing fields):', cleanData);
