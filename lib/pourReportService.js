...
export const cleanTimeFormat = (timeStr) => {
...
export const buildPourReportPayload = (data) =>
  removeUndefined({
  ...
  static async createPourReport(data) {
    try {
      ...
      if (typeof window !== 'undefined') {
        const response = await fetch('/api/pour-reports', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(body?.error || 'Failed to save pour report')
        }
