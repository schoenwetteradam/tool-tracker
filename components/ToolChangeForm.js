// components/ToolChangeForm.js - Complete Version with Auto-Timestamp for QR Code Integration
...
const ToolChangeForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    work_center: '',
    equipment_number: '', // Real equipment numbers from your system
    operation: '',
    part_number: '',
    job_number: '',
    ...
    old_tool_condition: '',
    pieces_produced: '',
    cycle_time_before: '', // Cycle time with old tool
    cycle_time_after: '',  // Cycle time with new tool
    
    // Quality & Throughput Metrics
    dimension_check: '',
    surface_finish: '',
    scrap_pieces: '', // Parts scrapped due to tool condition
    rework_pieces: '', // Parts requiring rework
    downtime_minutes: '', // Machine downtime for tool change
    
    // Improvement Tracking
    tool_life_expected: '', // Expected tool life in pieces
    tool_life_actual: '',   // Actual pieces produced
    feeds_speeds_changed: '', // Were feeds/speeds adjusted?
    coolant_condition: '',
    
    // Root Cause & Actions
    failure_mode: '', // How did the tool fail?
    corrective_action: '', // What was done to prevent recurrence?
    notes: ''
  });
  ...
  useEffect(() => {
    ...
    // Set up interval to update every minute while form is open
    const timeInterval = setInterval(updateCurrentTime, 60000); // Update every minute

    // Handle visibility change (when user returns to tab after scanning QR code)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Failed to submit tool change');
      }
  ...
  const shifts = [1, 2, 3];
  const workCenters = ['010', '040', '202', '203', '204', '205', '259']; // Added 010 (melting) and 040 (machining)
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Track tool changes to optimize throughput, quality, and overall equipment effectiveness</p>
      </div>
      ...
      {submitStatus && (
        <div className={`mb-6 p-4 rounded-lg flex items-center space-x-2 ${
          ...
          <span>
            {submitStatus === 'success' 
              ? 'Tool change recorded successfully! Data saved for performance analysis.' 
              : 'Error submitting form. Please check required fields and try again.'}
          </span>
        </div>
      )}
      ...
      <div className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            <Save size={20} />
            <span>{isSubmitting ? 'Saving Tool Change...' : 'Save Tool Change'}</span>
          </button>
        </div>
